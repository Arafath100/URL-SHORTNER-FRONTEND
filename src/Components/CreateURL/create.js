...
import { createURL, updateURLCount } from "../../Services/APIservices"; // Importing API services for URL operations

function CreateURL() {
  const [urlData, setURLData] = useState({}); // State to store URL data
  const [longURL, setLongURL] = useState([]); // State to manage long URL input
  const [error, setError] = useState(""); // State for displaying error messages

  // Function to create URL via API call
  ...
  // Handling URL submission to create a shortened URL
  function handleSubmit(e) {
    ...
    if (loggedUser || tokenAuth !== null) {
      ...
      createUrlDB(data, config); // Creating a shortened URL via createUrlDB function
    } else {
  ...
  // Checking if user is authenticated on component mount
  useEffect(() => {
    let tokenAuth = localStorage.getItem("tokenAuth");
    if (!tokenAuth) {
      window.alert("Log in to continue");
