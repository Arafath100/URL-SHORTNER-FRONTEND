...
const ResetPwd = () => {
  ...
  // State variables to manage password input and error/success messages
  const [pwd, setPwd] = useState("");
  const [pwd2, setPwd2] = useState("");
  const [error, setError] = useState(false);
  const [success, setSuccess] = useState(false);

  // Access the 'useNavigate' hook from React Router
  const navigate = useNavigate();

  // Function to reset the password by making an API call
